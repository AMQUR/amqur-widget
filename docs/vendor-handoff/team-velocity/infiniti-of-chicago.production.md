# Team Velocity — INFINITI of Chicago (production)

**Status: DO NOT PUBLISH until owner approves**

| Field | Value |
|-------|-------|
| Dealership name | INFINITI of Chicago |
| Tenant slug | `infiniti-of-chicago` |
| Location slug | `main` |
| API URL | `https://api.dialusnow.com` |
| Widget URL | `https://widget.dialusnow.com` |
| Logo URL | `https://widget.dialusnow.com/assets/tenants/infiniti-of-chicago/logo.3e97eead086a.svg` |
...
```html
<script defer src="https://widget.dialusnow.com/embed.js" data-tenant="infiniti-of-chicago" data-location="main" data-locale="en"></script>
```
...
<script defer src="https://widget.dialusnow.com/embed.js"
  data-tenant="infiniti-of-chicago"
  data-location="main"
...
- [ ] Script tag uses exact `data-tenant="infiniti-of-chicago"` and `data-location="main"`
...
- [ ] Exact origin allowlisted for `infiniti-of-chicago` only (other tenants unchanged)
...
4. Optionally clear `localStorage` keys matching `amqur_conversation_v1_infiniti-of-chicago_main` (and saved-VIN key if present).

## Emergency-disable instructions

1. **Site:** Pause GTM tag or remove script (fastest visitor-facing kill).
2. **API:** Clear / empty `allowedOrigins` for `infiniti-of-chicago` so tokens fail closed (AMQUR ops only — does not alter branding).
