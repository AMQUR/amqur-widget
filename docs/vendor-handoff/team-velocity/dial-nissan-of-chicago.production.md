# Team Velocity — Dial Nissan of Chicago (production)

**Status: DO NOT PUBLISH until owner approves**

| Field | Value |
|-------|-------|
| Dealership name | Dial Nissan of Chicago |
| Tenant slug | `dial-nissan-of-chicago` |
| Location slug | `main` |
| API URL | `https://api.dialusnow.com` |
| Widget URL | `https://widget.dialusnow.com` |
| Logo URL | `https://widget.dialusnow.com/assets/tenants/dial-nissan-of-chicago/logo.fd272741fd3c.svg` |
...
Rooftop package — activate only after exact origin verification.
...
```html
<script defer src="https://widget.dialusnow.com/embed.js" data-tenant="dial-nissan-of-chicago" data-location="main" data-locale="en"></script>
```
...
<script defer src="https://widget.dialusnow.com/embed.js"
  data-tenant="dial-nissan-of-chicago"
  data-location="main"
...
- [ ] Script tag uses exact `data-tenant="dial-nissan-of-chicago"` and `data-location="main"`
...
- [ ] Exact origin allowlisted for `dial-nissan-of-chicago` only (other tenants unchanged)
...
4. Optionally clear `localStorage` keys matching `amqur_conversation_v1_dial-nissan-of-chicago_main` (and saved-VIN key if present).

## Emergency-disable instructions

1. **Site:** Pause GTM tag or remove script (fastest visitor-facing kill).
2. **API:** Clear / empty `allowedOrigins` for `dial-nissan-of-chicago` so tokens fail closed (AMQUR ops only — does not alter branding).
