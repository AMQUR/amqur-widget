# Team Velocity — Jeep of Chicago (production)

**Status: DO NOT PUBLISH until owner approves**

| Field | Value |
|-------|-------|
| Dealership name | Jeep of Chicago |
| Tenant slug | `jeep-of-chicago` |
| Location slug | `main` |
| API URL | `https://api.dialusnow.com` |
| Widget URL | `https://widget.dialusnow.com` |
| Logo URL | `https://widget.dialusnow.com/assets/tenants/jeep-of-chicago/logo.65f09faa2c92.svg` |
...
Recommended first rooftop pilot after Dial Auto Group employee test.
...
```html
<script defer src="https://widget.dialusnow.com/embed.js" data-tenant="jeep-of-chicago" data-location="main" data-locale="en"></script>
```
...
- [ ] Script tag uses exact `data-tenant="jeep-of-chicago"` and `data-location="main"`
...
- [ ] Exact origin allowlisted for `jeep-of-chicago` only (other tenants unchanged)
...
4. Optionally clear `localStorage` keys matching `amqur_conversation_v1_jeep-of-chicago_main` (and saved-VIN key if present).

## Emergency-disable instructions

1. **Site:** Pause GTM tag or remove script (fastest visitor-facing kill).
2. **API:** Clear / empty `allowedOrigins` for `jeep-of-chicago` so tokens fail closed (AMQUR ops only — does not alter branding).
